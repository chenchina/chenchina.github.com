	...
	if(!empty($colabs_sbm_options['sidebars'])){
		...
		foreach($colabs_sbm_options['sidebars'] as $sidebar){
			...
			if(!empty($sidebar_piggy)) {
				...
				// $type = $colabs_sbm_options['sidebars'][$sidebar_id]['conditionals']['type']; // Resolves dependencies issue from V4.3.7 when commented out.
			} // End IF Statement
			...
			if ( ( $type == 'custom_post_type' && is_single() ) && in_array( $post->post_type, array_keys( $_post_types ) ) ) {
				...
				if( $post->post_type == $id && $sidebar_id == 'colabs_sbm_custom_post_type_' . $id . '_' . $current_sidebar_id ) {
					...
					if ( is_singular() && $sidebar_id == 'colabs_sbm_hierarchy_singular_' . $current_sidebar_id ) {
...
function colabsthemes_sbm_page(){
	...
	if ( isset( $_POST['colabs_save'] ) && $_POST['colabs_save'] == 'sbm_reset' ) {
	...
	jQuery(document).ready(function() {
		...
		jQuery( "#colabs-sbm-get-links").live( "submit",function(){
...
<div class="wrap colabs_container">
            <div class="themever left">
                ...
                <h2><?php echo $themename; ?> <?php echo $local_version; ?>&nbsp;<?php _e( 'Sidebar Settings' ) //your admin panel title ?></h2>
            </div>
         ...
         <div id="main">
         	<div id="content" class="sbm-content">
         		<?php /*
         		if(isset($colabs_sbm_options['settings']['infobox'])){
         		if($colabs_sbm_options['settings']['infobox'] == 'show'){ ?>
         		<div class="info-box">

         			<h2>CoLabsThemes Sidebar Settings</h2>

         			<p>You're one step closer to having total control over your theme. This Sidebar Settings is available only to themes running
         			version 3.0.0 of the CoLabsFramework. If you can see this message you're framework is up-to-date. Good Job!</p>
         			...
         			your theme manually</strong> if you're want to make use of this Sidebar Settings feature. Please take note of the following:</p>

         			<p><strong>Manual Installation:</strong> Replace all the <code>dynamic_sidebar</code> functions with the new
         			<code>colabs_sidebar</code> and replace all <code>is_active_sidebar</code> with the new <code>colabs_active_sidebar</code>. These are typically found in the
         		...
         		<div id="sbm-sidebar">
             		...
             		<div id="colabs-sbm-menu" class="postbox">
         				...
         				<li><a href="#">Page Templates <span>[+]</span></a>
	     					...
	     					foreach($page_templates as $name => $template){
	     						...
	     						//if(array_key_exists( 'colabs_sbm_page_template_'.$template,$colabs_sbm_options['sidebars'])){ continue; }
	     				...
	     				<li><a href="#"><?php echo ucwords(str_replace( '_',' ',$taxonomy)); ?> <span>[+]</span></a>
	     					...
	     					foreach($terms as $term){
	   	     					//if(array_key_exists ( 'colabs_sbm_'.$taxonomy.'_'.$term->term_id,$colabs_sbm_options['sidebars'])){ continue; }
	     				...
	     				<li><a href="#">Template Hierarchy <span>[+]</span></a>
	     					<ul>
	     						...
	     						foreach($heirarchy as $name => $item){
	   	     					//if(array_key_exists ( 'colabs_sbm_hierarchy_' . $item,$colabs_sbm_options['sidebars'])){ continue; }
	     			...
	     			<div class="colabs-sbm-builder">
	     				...
	     				<form action="" id="colabs-sbm-get-links">
							...
							<div id="colabs-sbm-builder-meta">
							    <div id="colabs-sbm-builder-meta-top">Template Info</div>
							...
							<div class="nav-tabs-nav">
	     						<div class="nav-tabs-wrapper">
									<div class="nav-tabs">
										<a id="colabs-sbm-tab-new" href="#" class="nav-tab nav-tab-active">Create a new Sidebar</a>
										<?php if(!empty($colabs_sbm_options['sidebars'])) { ?>
										<a id="colabs-sbm-tab-existing" class="nav-tab" href="#">Use Existing Sidebar</a>
										<?php } ?>
										<a id="colabs-sbm-toggle-info" class="fr" href="#">Template Info<img src="<?php get_template_directory_uri(); ?>/functions/images/ico-info.png" /></a>
									</div>
								</div>
							</div>
							<div class="builder-header">
								<label id="colabs-sbm-label-sb-name"><span>Sidebar Name</span> <input value="" type="text" name="sidebar-title" id="sidebar-title"/></label>
							</div>
							...
							<div id="colabs-sbm-builder-body">
							    ...
							    <div id="colabs-sbm-builder-part-create" class="colabs-sbm-builder-part-inner">
							    	...
							    	<label id="colabs-sbm-label-sb-desc"><span>Sidebar description</span> <textarea id="sidebar-description" name="sidebar-description" style="width:230px"></textarea></label>
		 ...
		 <form action="" method="post" style="display:inline" id="colabsform-reset">
            ...
            <input name="reset" type="submit" value="Reset Sidebar Settings" class="button submit-button reset-button" onclick="return confirm( 'Click OK to reset. Any Sidebar Settings settings will be lost!' );" />
...
function colabs_sbm_callback() {
	...
	if($save_type == 'colabs_sbm_get_links'){
	...
	if($save_type == 'colabs_sbm_add_sidebar'){
	...
	if($save_type == 'colabs_sbm_delete-sidebar'){
		$id = $_POST['data'];
		$ids = array();
	...
	if($save_type == 'colabs_sbm_save-sidebar'){
	...
	if($save_type == 'colabs_sbm_dismiss_intro'){
