...
function colabsthemes_framework_update_page(){
        ...
        ?>
            ...
            <form method="post"  enctype="multipart/form-data" id="colabsform" action="<?php /* echo $url; */ ?>">
                ...
                <h3>A new version of ColorLabs Framework is available.</h3>
                <p>This updater will download and extract the latest ColorLabs Framework files to your current theme's functions folder. </p>
                <p>We recommend backing up your theme files before updating.</p>
                <p>&rarr; <strong>Your version:</strong> <?php echo $upd['localversion']; ?></p>

                <p>&rarr; <strong>Current Version:</strong> <?php echo $upd['remoteversion']; ?></p>

                <input type="submit" class="button" value="Update Framework" />
                <?php } else { ?>
                <h3>You have the latest version of ColorLabs Framework</h3>
                <p>&rarr; <strong>Your version:</strong> <?php echo $upd['localversion']; ?></p>
                <?php } ?>
...
function colabsthemes_framework_update_check(){
    ...
    $upd['localversion'] = get_option( 'colabs_framework_version' );
    $upd['remoteversion'] = colabs_get_fw_version();
    
    // Test if new version
    $upd['update'] = false;
    $loc = explode( '.',$upd['localversion']);
    $rem = explode( '.',$upd['remoteversion']);
		...
		if ( is_wp_error($dounzip) ) {
			...
			if($error == 'incompatible_archive') {
				...
				function colabsthemes_framework_update_no_archive_warning() {
					echo "<div id='colabs-no-archive-warning' class='updated fade'><p>Failed: Incompatible archive</p></div>";
				}
			...
			if($error == 'empty_archive') {
				function colabsthemes_framework_update_empty_archive_warning() {
					echo "<div id='colabs-empty-archive-warning' class='updated fade'><p>Failed: Empty Archive</p></div>";
				}
			...
			if($error == 'mkdir_failed') {
				function colabsthemes_framework_update_mkdir_warning() {
					echo "<div id='colabs-mkdir-warning' class='updated fade'><p>Failed: mkdir Failure</p></div>";
				}
			...
			if($error == 'copy_failed') {
				function colabsthemes_framework_update_copy_fail_warning() {
					echo "<div id='colabs-copy-fail-warning' class='updated fade'><p>Failed: Copy Failed</p></div>";
				}
...
function colabsthemes_timthumb_update_page(){
        ...
        $url = admin_url( 'admin.php?page=colabsthemes_framework_update' );
       	...
       	// Do the update
       	if (isset($_POST['colabs_update_save'])) {
       		
       		// Read in the old file
       		$filename = locate_template( 'scripts/timthumb.php' );
       		
       		// If File exists
       		if ( $filename != '' ) {
       			
       			// Call function test
				$file_read = colabs_check_if_thumbs_are_equal($filename);
				$file_open = true;
       			$file_write = false;
				...
				if ($file_read) {
					...
					if ($file_open) {
						// New File Contents
						$new_file_contents = colabs_thumb_new_contents();
					...
					if ($file_open && $file_write) {
						update_option('colabs_timthumb_update', 'true');
					} // End If Statement
				} else {
					echo 'An error occurred while reading your current timthumb.php';
				} // End If Statement
        ...
        ?>
            ...
            <form method="post"  enctype="multipart/form-data" id="colabsform" action="<?php /* echo $url; */ ?>">
                ...
                <p>This updater will remove the old version of TimThumb (timthumb.php) in your theme folder, and use the new TimThumb in the ColorLabs Framework.</p>
...
function colabs_check_if_thumbs_are_equal($filename, $override = false) {
...
function colabs_thumb_new_contents() {
	// New File Contents
    $new_file_contents = 
"<?php" . "\n" . "/*" . "\n" . "\n" . "TIMTHUMB.PHP HAS MOVED" . "\n" . "" . "\n" . "TimThumb (timthumb.php) has been moved to the ColorLabs Framework (/functions/timthumb.php)." . "\n" . "\n" . "You can create a config file in your theme folder called timthumb-config.php and" . "\n" . "copy over any definitions that you want to customize." . "\n" . "\n" . "*/" . "\n" . "echo 'TimThumb (thumb.php) is now in the <a href=\"functions/thumb.php\">CoLabsFramework</a>';" . "\n" . "?>";
