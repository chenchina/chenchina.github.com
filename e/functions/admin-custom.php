...
function colabsthemes_metabox_create($post,$callback) {
    ...
    if(get_option( 'seo_colabs_hide_fields') != 'true' AND $template_to_show == 'seo'){
    ...
    foreach ($colabs_metaboxes as $colabs_metabox) {
    	...
    	if ($type_selector) {
    		...
    		if(
        	        $colabs_metabox['type'] == 'text'
			OR      $colabs_metabox['type'] == 'select'
			OR      $colabs_metabox['type'] == 'select2'
			OR      $colabs_metabox['type'] == 'checkbox'
			OR      $colabs_metabox['type'] == 'textarea'
			OR      $colabs_metabox['type'] == 'calendar'
			OR      $colabs_metabox['type'] == 'time'
			OR      $colabs_metabox['type'] == 'radio'
			OR      $colabs_metabox['type'] == 'images') {
        	    ...
        	    elseif($colabs_metabox['type'] == 'text'){
        	    ...
        	    elseif ($colabs_metabox['type'] == 'textarea'){
        	    ...
        	    elseif ($colabs_metabox['type'] == 'calendar'){
        	    ...
        	    elseif ($colabs_metabox['type'] == 'time'){
        	    ...
        	    elseif ($colabs_metabox['type'] == 'select'){
        	    ...
        	    elseif ($colabs_metabox['type'] == 'select2'){
        	    ...
        	    elseif ($colabs_metabox['type'] == 'checkbox'){

        	        if($colabs_metaboxvalue == 'true') { $checked = ' checked="checked"';} else {$checked='';}
        	    ...
        	    elseif ($colabs_metabox['type'] == 'radio'){
				...
				elseif ($colabs_metabox['type'] == 'images')
				{
        	    ...
        	    elseif($colabs_metabox['type'] == 'upload')
        	    {
...
function colabsthemes_uploader_custom_fields($pID,$id,$std,$desc){
    ...
    if ( $href )
		$uploader .= '<span class="colabs_metabox_desc description">'.$desc.'</span></td>'."\n".'<td class="colabs_metabox_image"><a href="'. $upload .'"><img src="'.get_template_directory_uri().'/thumb.php?src='.$href.'&w=150&h=80&zc=1" alt="" /></a>';
...
function colabsthemes_metabox_handle(){
    ...
    if ( isset( $_POST['action'] ) && $_POST['action'] == 'editpost' ) {
        ...
        foreach ($colabs_metaboxes as $colabs_metabox) { // On Save.. this gets looped in the header response and saves the values submitted
			...
			OR $colabs_metabox['type'] == 'checkbox'
			OR $colabs_metabox['type'] == 'textarea'
			OR $colabs_metabox['type'] == 'images' ) // Normal Type Things...
            {
				...
				} elseif ( ! isset( $_POST[$var] ) && $colabs_metabox['type'] == 'checkbox' ) {
            ...
            } elseif( $colabs_metabox['type'] == 'upload' ) { // So, the upload inputs will do this rather
...
function colabsthemes_metabox_add() {
    ...
    if ( function_exists( 'add_meta_box') ) {
    	...
    	if ( function_exists( 'get_post_types') ) {
            ...
            if ( !empty($colabs_metabox_settings) ){
...
.colabs_input_checkbox { margin:0 10px 0 0; }
.colabs_input_radio { margin:0 10px 0 0; }
.colabs_input_radio_desc { font-size: 12px; color: #666 ; }
