...
function colabsthemes_add_admin() {
   	...
   	if ( isset( $_REQUEST['page'] ) ) {
		...
		if ( $_action == 'reset' ) {
			...
			if ( function_exists( 'check_ajax_referer' ) ) {
				if ( $_page == 'colabsthemes_seo' ) {
					check_ajax_referer( 'colabsframework-seo-options-reset', '_ajax_nonce' );
				} else if ( $_page == 'colabsthemes_layout_settings' ) {
			...
			switch ( $_page ) {
				case 'colabsthemes_options':
					...
					header( "Location: admin.php?page=colabsthemes_options&reset=true" );
					die;
				break;
	...
	$colabslayout = '';
    $layoutfile = TEMPLATEPATH . '/includes/theme-options-layout.php';
    ...
    */
    ...
    /*$colabsframeworksettings = '';
    if($super_user == $current_user_id || empty($super_user)) {
		$colabsframeworksettings = add_submenu_page( 'colabsthemes', 'Framework Settings', 'Framework Settings', 'manage_options', 'colabsthemes_framework_settings', 'colabsthemes_framework_settings_page' );
	}
    */
    ...
    if($super_user == $current_user_id || empty($super_user)) {
         $colabsthemepage = add_submenu_page( 'colabsthemes', 'CoLabsFramework Update', 'Update Framework', 'manage_options', 'colabsthemes_framework_update', 'colabsthemes_framework_update_page' );
    }

	// Update Timthumb Menu Item
    /*$file_located = locate_template( 'thumb.php' );
    if ($file_located != '') {
    	$file_test = colabs_check_if_thumbs_are_equal($file_located, true);
    } else {
    	$file_test = false;
    }
    ...
    if( ( $super_user == $current_user_id || empty($super_user) ) && $timthumb_update == '' && $file_test) {
         $colabsthemepage = add_submenu_page( 'colabsthemes', 'Timthumb Update', 'Update Timthumb', 'manage_options', 'colabsthemes_timthumb_update', 'colabsthemes_timthumb_update_page' );
    }*/
    
	// Buy Themes Menu Item
    /*if(get_option( 'framework_colabs_buy_themes_disable') != 'true') {
	...
	add_action( "admin_print_scripts-$colabsframeworksettings", 'colabs_load_only' );
	...
	add_action( "load-$colabsframeworksettings", 'colabs_nonajax_callback' );
	add_action( "load-$colabslayout", 'colabs_nonajax_callback' );
	add_action( "load-$colabsseo", 'colabs_nonajax_callback' );
	// add_action( "load-$colabssbm", 'colabs_nonajax_callback' );
...
function colabs_reset_options($options,$page = ''){
	...
	if( $page == 'colabsthemes_options' ){
		delete_option( 'colabs_options' );
	}
    if( $page == 'colabsthemes_layout_settings' ){
...
function colabs_load_only() {
    ...
    ?>  
    <link rel="stylesheet" type="text/css"  href="<?php echo get_template_directory_uri(); ?>/<?php echo $_wp_admin_css_colors['name']; ?>.css" />   
    <?php
		 // COLOR Picker ?>
		...
		<script type="text/javascript">
			...
			jQuery( '#colabsform').submit(function(){
					...
					var data = {
						...
						type: 'framework',
						<?php } ?>
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'colabsthemes_layout_settings'){ ?>
						type: 'layout',
						<?php } ?>                        
...
global $wpdb; // this is how you get access to the database
	...
	elseif ($save_type == 'options' OR $save_type == 'seo' OR $save_type == 'tumblog' OR $save_type == 'framework' OR $save_type == 'layout') {
		...
		if ( isset( $output['colabs_save'] ) && $output['colabs_save'] == 'reset' ) { unset( $output['colabs_save'] ); }
	...
	if( $save_type == 'options' OR $save_type == 'framework' OR $save_type == 'layout' ){
...
if ( ! function_exists( 'colabs_nonajax_callback' ) ) {
	function colabs_nonajax_callback() {
		if ( isset( $_POST['_ajax_nonce'] ) && isset( $_POST['colabs_save'] ) && ( $_POST['colabs_save'] == 'save' ) ) {
...
/*-----------------------------------------------------------------------------------*/
global $pagenow;
if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] == 'colabsthemes' && get_option( 'framework_colabs_theme_version_checker' ) == 'true' ) {
